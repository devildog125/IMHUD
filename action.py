...
class HelmetCommand(object):
    # Handles Helmet commands via serial
    ...
    def run(self, voice_command):
        ...
        if self.command == "open":
            ...
            port.write(b'2\n')
        elif self.command == "open faceplate":
            self.say("Opening faceplate")
            port.write(b'1\n')
        elif self.command == "close faceplate":
            self.say("Close faceplate")
            port.write(b'2\n')
        elif self.command == "eyes off":
            port.write(b'3\n')
        elif self.command == "eyes on":
            port.write(b'4\n')
        elif self.command == 'intro':
            port.write(b'5\n')
            self.say("Playing Intro")
        elif self.command == 'reboot':
            port.write(b'6\n')
            self.say("Rebooting all systems")
        elif self.command == "stop camera":
            port.write(b'7\n')
            self.say("Stopping camera")
        elif self.command == "start camera":
            port.write(b'8\n')
            self.say("Starting camera")
        else:
            logging.error("Error identifying helmet command.")
            self.say("Sorry I didn't identify that command")
...
def make_actor(say):
    ...
    # =========================================
    # Jarvis interactions go here!
    # =========================================

    actor.add_keyword(_('open'), HelmetCommand(say, 'open'))
    actor.add_keyword(_('close'), HelmetCommand(say, 'close'))
    actor.add_keyword(_('open faceplate'), HelmetCommand(say, 'open'))
    actor.add_keyword(_('close faceplate'), HelmetCommand(say, 'close'))
    actor.add_keyword(_('eyes on'), HelmetCommand(say, 'eyes on'))
    actor.add_keyword(_('eyes off'), HelmetCommand(say, 'eyes off'))
    actor.add_keyword(_('play intro'), HelmetCommand(say, 'intro'))
    actor.add_keyword(_('reboot the system'), HelmetCommand(say,'reboot'))
    actor.add_keyword(_('stop camera'), HelmetCommand(say, 'stop camera'))
    actor.add_keyword(_('start camera'), HelmetCommand(say, 'start camera'))
    actor.add_keyword(_('turn auxiliary lights off'), HelmetCommand(say, 'eyes off'))
    actor.add_keyword(_('turn auxiliary lights on'), HelmetCommand(say, 'eyes on'))
